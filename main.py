...
def add_doctor(data: dict, token=Depends(verify_token)):
    ...
    cursor = db.cursor()
    cursor.execute(
        "INSERT INTO doctors (name, specialty, experience, city) VALUES (%s, %s, %s, %s)",
        (data["name"], data["specialty"], data["experience"], data["city"])
    )
    db.commit()
    db.close()
    return {"message": "Doctor added!"}

# Doctor Update — Admin Only
...
def update_doctor(doctor_id: int, data: dict, token=Depends(verify_token)):
    ...
    cursor = db.cursor()
    cursor.execute(
        "UPDATE doctors SET name=%s, specialty=%s, experience=%s, city=%s WHERE id=%s",
        (data["name"], data["specialty"], data["experience"], data["city"], doctor_id)
    )
    db.commit()
    db.close()
    return {"message": "Doctor updated!"}

# Doctor Delete — Admin Only
...
def add_patient(data: dict, token=Depends(verify_token)):
    ...
    cursor.execute(
        ...
        (name, age, disease, city, mobile, address, blood_group) 
        VALUES (%s, %s, %s, %s, %s, %s, %s)""",
        (data["name"], data["age"], data["disease"], data["city"],
         data.get("mobile", ""), data.get("address", ""), data.get("blood_group", ""))
    )
    db.commit()
    db.close()
...
def add_appointment(data: dict, token=Depends(verify_token)):
    ...
    cursor.execute(
        ...
        (patient_id, doctor_id, date, status, treatment, charges) 
        VALUES (%s, %s, %s, %s, %s, %s)""",
        (data["patient_id"], data["doctor_id"], data["date"], 
         data["status"], data["treatment"], data["charges"])
    )
    # Appointment id मिळवणे
    apt_id = cursor.lastrowid
    
    # Bill आपोआप तयार करणे
    cursor.execute(
...
def update_appointment(apt_id: int, data: dict, token=Depends(verify_token)):
    ...
    cursor.execute(
        """UPDATE appointments 
        SET status=%s, treatment=%s, charges=%s 
        WHERE id=%s""",
...
def pay_bill(bill_id: int, token=Depends(verify_token)):
    ...
    cursor = db.cursor()
    cursor.execute(
        "UPDATE bills SET payment_status='Paid' WHERE id=%s", (bill_id,)
    )
    db.commit()
    db.close()
...
def update_patient(patient_id: int, data: dict, token=Depends(verify_token)):
    ...
    cursor.execute(
        """UPDATE patients 
        SET name=%s, age=%s, disease=%s, city=%s, 
            mobile=%s, address=%s, blood_group=%s 
        WHERE id=%s""",
        (data["name"], data["age"], data["disease"], data["city"],
         data.get("mobile", ""), data.get("address", ""), 
         data.get("blood_group", ""), patient_id)
    )
    db.commit()
    db.close()
...
# Bed Assign — Patient ला bed द्यायला
...
def assign_bed(bed_id: int, data: dict, token=Depends(verify_token)):
    ...
    cursor = db.cursor()
    cursor.execute(
        "UPDATE beds SET status='Occupied', patient_id=%s WHERE id=%s",
        (data["patient_id"], bed_id)
    )
    db.commit()
    db.close()
...
def free_bed(bed_id: int, token=Depends(verify_token)):
    ...
    cursor = db.cursor()
    cursor.execute(
        "UPDATE beds SET status='Available', patient_id=NULL WHERE id=%s",
        (bed_id,)
    )
    db.commit()
    db.close()
